...
function makePlan(ast, headerContext, report) {
  ...
  // SELECT * has no expression node; its exact projection is supplied by report.columns.
  const computed = ast.select.filter(x => x.expression && ((x.expression.type !== 'column' && !containsAggregate(x.expression)) || (x.expression.type === 'call' && x.expression.name === 'SUM' && x.expression.args[0] && x.expression.args[0].type === 'case')));
  ...
  let n = 0; for (const join of ast.joins) { if (!collapsedLookups.has(join)) { n++; steps.push({ name: `Joined${n}`, kind: 'join', join, subplan: join.table.subquery && makePlan(join.table.subquery, headerContext, { columns: join.table.subquery.select.map(x => ({ name: x.alias })) }) }); steps.push({ name: `Expanded${n}`, kind: 'expand', join }); } }
...
function collectTypes(ast, context) { const out = []; const tables = (context || {}).tables || {}; const needs = e => e && (e.type === 'cast' || (e.type === 'call' && ['STRFTIME','JULIANDAY'].includes(e.name)) || (e.type === 'binary' && ['+','-','*','/'].includes(e.op))) || (e && Object.values(e).some(v => Array.isArray(v) ? v.some(needs) : v && typeof v === 'object' && needs(v)));
  for (const item of ast.select) if (needs(item.expression) && item.expression.type === 'column') { const c = ((tables[item.expression.resolvedTable] || {}).columns || []).find(x => (x.name || x) === item.expression.name); const type = String((c && c.internalType) || '').toLowerCase(); const header = item.expression.resolvedHeader || item.expression.name; out.push([header, /date|fecha/.test(header.toLowerCase()) ? 'date' : (/int|float|decimal|number/.test(type) ? 'number' : 'text')]); } return out; }
