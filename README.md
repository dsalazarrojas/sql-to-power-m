...
`SUM`/`AVG`/`MIN`/`MAX` accept any expression that isn't itself an aggregate — arithmetic, date math (`julianday(a) - julianday(b)`), `CASE`, `CAST` — directly inside the aggregate, with no staging subquery required; the same goes for `COUNT(DISTINCT expr)`. A `JOIN ... ON a = b AND c = d` also works directly: the equality becomes the join key and the rest of the condition becomes a filter right after that join.

`NOT EXISTS (SELECT 1 FROM t WHERE t.fk = outer.id)` / `EXISTS (...)` compile automatically when the subquery is a single correlated equality against one inner table with no `JOIN` of its own — the compiler rewrites it into the equivalent `LEFT JOIN (...) ON ... WHERE ... IS [NOT] NULL` anti-join before validating. Wider correlation (a `JOIN` inside the `EXISTS` subquery, more than one correlated column, or `EXISTS` under `OR`) is rejected with a hint to write that anti-join by hand — the compiler already supports it natively, `EXISTS` is sugar over the same mechanism for the common case.

When a query hits the `join fan-out` rejection, the app also tries [`src/compiler/suggest-fanout-rewrite.js`](src/compiler/suggest-fanout-rewrite.js) and, if it can safely reshape the SQL (pre-aggregate each joined branch, then join and wrap in an outer query), shows a "Suggested rewrite" block with a copy button under the rejection. It never compiles anything itself — it hands back SQL text to review and paste back into the editor, then recompile through the same real compiler. It bails with a plain-language reason instead of guessing on anything more complex (an aggregate mixing two branches, a compound/cross-alias join key, correlated scalar subqueries in `SELECT`, etc.).
